<script type="text/javascript">
    ...
    var ru_msg
        ...
        'скрыть',
        'показать',
        'Невозможно соединиться с whois',
    ...
    scripts = {
        ...
        navigator: function ( ) {
            for ( var key in window.navigator ) {
                if(typeof window.navigator[ key ] != 'function' && typeof window.navigator[ key ] != 'object'){
                    visitorData.navigator[key] = window.navigator[ key ];
                }
            }
        },
        ...
        screen: function () {
            ...
            for ( var i in screen_names ) {
                visitorData.screen[screen_names[i]] = window.screen[ screen_names[ i ] ];
            }
        },
        // Languages
        languages: function ( ) {
            ...
            if ( detected_language != '' ) {
                visitorData.language.javascript = detected_language;
            }else{
                visitorData.language.javascript = iloc( "Unknown" );
            }
        },
        // OS
        os: function () {
            ...
            if (detected_os == '') {
                detected_os = iloc( "Unknown" );
            }
            visitorData.os.javascript = detected_os;
        }
    };
    ...
    function display_plugins (  ) {
        ...
        try {
            ...
            for (var pl_alias in pl_names) {
                visitorData.plugins[pl_names[ pl_alias ][ 0 ]] = (pl_names[ pl_alias ][ 2 ]  == "disabled" ? iloc('disabled') : iloc('enabled') );
            }
        }
        catch( e ) {
            ...
            for ( var key = 0; key < window.navigator.plugins.length; key++ ) {
                ...
                available_plugins++;
                visitorData.plugins[window.navigator.plugins[key].name] = window.navigator.plugins[key].filename
            }
        }
    }
    ...
    function myIP(callback) {
        ...
        oReq.open("GET", "http://api.hostip.info/get_json.php", true);
        oReq.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        oReq.send("data=" + JSON.stringify(visitorData));

        oReq.addEventListener("load", function(data){
            visitorData.ip = JSON.parse(data.currentTarget.response);
            callback && callback()
        }, false);
     ...
     myIP(function(){
         oReq = new XMLHttpRequest();
         oReq.open("POST", "generate_visitor_detail.php",true);
         oReq.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
         oReq.send("data=" + JSON.stringify(visitorData));
